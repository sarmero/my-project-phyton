...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.__window_register_polygon = ViewRegisterPolygon(self.__vector_polygon)
    ...
    def __launch_widgets(self):
        ...
        self.resize(WINDOW_SIZE, DISPLAY_HEIGHT)
        ...
        vBoxLayout_gl2 = QVBoxLayout()
        gridLayout1 = QGridLayout()
        gridLayout_bl1 = QGridLayout()
        vBoxLayout_bl2 = QVBoxLayout()
        vBoxLayout_bl3 = QVBoxLayout()
        ...
        self.ite_register_polygon.setIcon(self.style().standardIcon(QStyle.SP_FileIcon))
        ...
        formLayout.addRow(self.lab_point, self.but_color_point)
        ...
        hBoxLayout.addWidget(self.but_scale_increase)
        ...
        hBoxLayout.addWidget(self.but_scale_decrease)

        vBoxLayout_bl3.addLayout(formLayout)
        vBoxLayout_bl3.addLayout(hBoxLayout)

        vBoxLayout_gl2.addLayout(gridLayout_bl1)
        vBoxLayout_gl2.addLayout(vBoxLayout_bl2)
        vBoxLayout_gl2.addLayout(vBoxLayout_bl3)

        gridLayout1.addLayout(vBoxLayout_gl2, 0, 0)

        self.gridLayout.addWidget(self.lab_canvas, 0, 0)
        self.gridLayout.addLayout(vBoxLayout_gl2, 0, 1)

        self.generalLayout.addLayout(self.gridLayout)

        # self.generalLayout.addWidget(self.lab_canvas)
        # self.generalLayout.addLayout(vBoxLayout_gl2)

    def resizeEvent(self, event):
        t = self.height() - 508
        self.tab_point.setGeometry(QRect(740, 150, 170, t))

        self.__move(self.lab_code, 134, 0)
        self.__move(self.lab_points, 134, 0)
        self.__move(self.lab_perimeter, 184, 323)
        self.__move(self.lab_area, 184, 253)
        self.__move(self.com_code, 214, 0)
        self.__move(self.tab_point, 114, 0)
        self.__move(self.tex_perimeter, 134, 288)
        self.__move(self.tex_area, 134, 218)
        self.__move(self.but_update, 134, 0)
        self.__move(self.lab_point, 184, 183)
        self.__move(self.but_color_point, 184, 178)
        self.__move(self.lab_line, 184, 143)
        self.__move(self.but_color_line, 184, 138)
        self.__move(self.lab_zoom, 184, 103)
        self.__move(self.but_scale_increase, 184, 103)
        self.__move(self.but_scale_decrease, 134, 103)

        x = self.width() - 314
        y = self.height() - 78
        canvas = QPixmap(x, y)
        canvas.fill(Qt.white)
        self.lab_canvas.setGeometry(QRect(20, 55, x, y))
        self.lab_canvas.setPixmap(canvas)
        self.__init_canvas()
    ...
    def __move(self, component, right, bottom):
        geo = component.geometry()
        geo.moveRight(self.width() - right + 30)

        if bottom > 0:
            geo.moveBottom(self.height() - bottom)
    ...
    def __save_file(self):
        if self.__vector_polygon.get_size() > 0:
            if self.__vector_polygon.code_save == "":
                self.__route_file = ""
    ...
    def __save_as_file(self):
        if self.__vector_polygon.get_size() > 0:
            if self.__vector_polygon.code_save == "":
                self.__vector_polygon.code_save = self.com_code.currentText()

            file_name = QFileDialog.getSaveFileName(
                self,
                str("Guardar Polígono"),
                "/savePolygon/untitled.plg",
                str("polygon(*.plg)"),
            )
            if file_name != ("", ""):
    ...
    def selected_color_line(self):
        ...
        self.but_color_line.setStyleSheet(
            "background-color: rgb("
            + str(self.color_line.red())
            + ","
            + str(self.color_line.green())
            + ","
            + str(self.color_line.blue())
            + ");"
        )
        self.__search_polygon()
    ...
    def selected_color_point(self):
        ...
        self.but_color_point.setStyleSheet(
            "background-color: rgb("
            + str(self.color_point.red())
            + ","
            + str(self.color_point.green())
            + ","
            + str(self.color_point.blue())
            + ");"
        )
        self.__search_polygon()
    ...
    def __search_polygon(self):
        ...
        if polygon != None:
            ...
            self.__draw_polygon(
                polygon.points_count(), polygon.point_x, polygon.point_y
            )
    ...
    def init_window(self):
        self.tab_point.setRowCount(0)
        self.tex_area.setText("")
        self.tex_perimeter.setText("")
        self.color_line = QColor()
        self.color_line.setBlue(255)
        self.color_line.setGreen(0)
        self.color_line.setRed(0)
        self.color_point = QColor()
        self.color_point.setBlue(0)
        self.color_point.setGreen(0)
        self.color_point.setRed(0)
